...
struct GarbageCollectionOperationTest : Test, DistributorStripeTestUtil {
    ...
    void enable_two_phase_gc() {
        ...
        config->set_enable_two_phase_garbage_collection(true);
...
TEST_F(GarbageCollectionOperationTest, two_phase_gc_requires_config_enabling_and_explicit_node_support) {
    ...
    config = make_config();
    config->set_enable_two_phase_garbage_collection(false);
    configure_stripe(std::move(config));
...
TEST_F(GarbageCollectionOperationTest, no_second_phase_if_first_phase_has_results_but_intersection_is_empty) {
    ...
    ASSERT_EQ(0u, _sender.commands().size());
    EXPECT_TRUE(op->ok()); // Still not a failure to have nothing to do
    // In this case as well, GC timestamp must be updated so we can move on to another bucket.
    EXPECT_NO_FATAL_FAILURE(assert_bucket_last_gc_timestamp_is(_gc_start_time_sec));
}
...
struct GarbageCollectionOperationPhase1FailureTest : GarbageCollectionOperationTest {
    ...
    void receive_phase1_replies_and_assert_no_phase_2_started() {
        ...
        ASSERT_EQ(0u, _sender.commands().size());
        EXPECT_FALSE(_op->ok());
        // GC not completed, so timestamp _not_ updated
        EXPECT_NO_FATAL_FAILURE(assert_bucket_last_gc_timestamp_is(0));
    }
};
...
TEST_F(GarbageCollectionOperationPhase1FailureTest, no_second_phase_if_cluster_state_version_changed_between_phases) {
    enable_cluster_state("version:11 distributor:1 storage:2"); // 10 -> 11
    receive_phase1_replies_and_assert_no_phase_2_started();
}
...
/*
 * TODO test
 *  - [DONE] second phase not started until first phase is complete
 *  - [DONE] no second phase if first phase failure
 *  - [DONE] no second phase if
 *     - [DONE] cluster state changed, or
 *     - [DONE] pending cluster state, or
 *     - [DONE] bucket now gone/inconsistently split
 *  - [DONE] second phase sends intersection of entries
 *  - [DONE] documents with write lock already held are not included in remove set
 *  - [DONE] no second phase if intersection is empty
 *    - [DONE] gc timestamp updated since no docs to gc == completed for this period
 *  - gc timestamp updated after phase two complete
 *  - metrics updated after phase 2 done
 *  - operation priority
 */
